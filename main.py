...
async def debug_resume_parse(
    resume: UploadFile = File(...)
):
    """
    Extract text content from uploaded PDF resume for debugging purposes.

    - **resume**: PDF resume file to parse
    ...
    if resume.size and resume.size > 50 * 1024 * 1024:  # 50MB limit
        raise HTTPException(
            status_code=400,
            detail="File size too large. Maximum 50MB allowed."
        )
    ...
    request_id = str(uuid.uuid4())
    pdf_path = TEMP_DIR / f"{request_id}_debug.pdf"
    ...
    try:
        ...
        print(f"Saved PDF file for debug: {pdf_path} ({len(content)} bytes)")
        ...
        if not extracted_text.strip():
            extracted_text = "No text content could be extracted from this PDF. The file might contain only images or be password protected."
        ...
        return {
            ...
            "pages_processed": len(pdf_reader.pages) if 'pdf_reader' in locals() else 0
        }
